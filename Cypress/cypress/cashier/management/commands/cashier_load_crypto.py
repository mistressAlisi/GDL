...
from cashier.models import CryptoCurrencies
from parameters.models import VHost, VHostMenuEntry, VHostCryptoExchangeCurrencies


class Command(BaseCommand):
    help = 'Import a Crypto Currencies data file for the specified vhost'
    ...
    def handle(self, *args, **options):
        ...
        with open(options["in_file"][0],"r") as fh:
            ...
            for entry in imported_data.dict:
                vCryptoObj,cc = CryptoCurrencies.objects.get_or_create(vhost=vHost,symbol=entry["symbol"])
                if cc:
                    vCryptoObj.save()
