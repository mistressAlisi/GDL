...
from dataengine.consumer.main import DataEngineConsumerHTTPd
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.MIGRATE_HEADING(
                f"DataEngine DataConsumerDaemon  Async Daemon starting for {vHost}"
            )
        )
        ...
        # --- Initialize Engine ---
        engine = DataEngineConsumerHTTPd(vhost=vHost, logger=logging.getLogger("DataConsumerDaemon"),
                                         remote_host_url=options["endpoint"][0],api_key=options["apikey"][0])
