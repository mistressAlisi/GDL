...
class AMFGame(models.Model):
    ...
    def __str__(self):
        return f"AMF Game: {self.id}: {self.away_team.name} at {self.home_team.name} Status: {self.status_short}. Event Time: {self.commence_time}"

@admin.register(AMFGame)
class AMFGameAdmin(admin.ModelAdmin):
    list_filter = ["stage","week","status_short","home_team","away_team","commence_time"]
    search_fields = ["id"]


class AMFGameScore(models.Model):
    class Meta:
        unique_together = (("match","team","vhost"))
        ordering = ["match","team"]
    uuid = models.UUIDField(primary_key=True,default=uuid.uuid4)
    vhost = models.ForeignKey('parameters.VHost', on_delete=models.CASCADE)
    match = models.ForeignKey(AMFGame, on_delete=models.RESTRICT, null=True)
    ...
    def __str__(self):
        return f"Scores for AMF Game {self.match.id}: Team: {self.team.name} "

@admin.register(AMFGameScore)
class AMFGameScoreAdmin(admin.ModelAdmin):
    list_filter = ["match","team"]

class AMFGameScoreSummary(models.Model):
    uuid = models.UUIDField(primary_key=True,default=uuid.uuid4)
    vhost = models.ForeignKey('parameters.VHost', on_delete=models.CASCADE)
    match = models.ForeignKey(AMFGame, on_delete=models.RESTRICT, null=True)
    team = models.ForeignKey(Team, on_delete=models.RESTRICT, null=True)
...
@admin.register(AMFGameScoreSummary)
class AMFGameScoreSummaryAdmin(admin.ModelAdmin):
    list_filter = ["match","team"]


class AMFGamePlayerStats(models.Model):
    class Meta:
    ...
    amf_game = models.ForeignKey(AMFGame, on_delete=models.RESTRICT)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def __str__(self):
        return f"{self.player} (Season: {self.season}) (Group: {self.group}): Game {self.amf_game}: {self.name}: {self.value}"

@admin.register(AMFGamePlayerStats)
class AMFGamePlayerStatsAdmin(admin.ModelAdmin):
    list_filter = ["season","group","amf_game"]
    search_fields = ["name","group","player__name"]

class AMFPlayerStats(models.Model):
    class Meta:
...
@admin.register(AMFPlayerStats)
class AMFPlayerStatsAdmin(admin.ModelAdmin):
    list_filter = ["season", "group"]
    search_fields = ["name", "group", "player__name"]


class AMFGameEvent(models.Model):
    uuid = models.UUIDField(primary_key=True,default=uuid.uuid4)
    vhost = models.ForeignKey('parameters.VHost', on_delete=models.CASCADE)
    amf_game = models.ForeignKey(AMFGame, on_delete=models.RESTRICT)
    ...
    def __str__(self):
        return f"{self.amf_game}: {self.team.name}->{self.player.name}: {self.type} @ {self.minute}: {self.comment}"



@admin.register(AMFGameEvent)
class AMFGameEventAdmin(admin.ModelAdmin):
    list_filter = ["amf_game", "quarter","minute","team"]
    search_fields = ["name", "group", "player__name"]
