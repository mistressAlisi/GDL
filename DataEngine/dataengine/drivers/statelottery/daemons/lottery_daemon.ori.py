...
class LotteryScrapeDaemon:
    ...
    @sync_to_async
    def _get_active_games(self) -> List[LotteryGame]:
        """Get all active lottery games."""
        return list(LotteryGame.objects.filter(active=True))

    @sync_to_async
    def _get_game_by_slug(self, slug: str) -> Optional[LotteryGame]:
        """Get a game by its slug."""
        try:
            return LotteryGame.objects.get(slug=slug)
        except LotteryGame.DoesNotExist:
            return None

    def _should_scrape(self, game: LotteryGame, now: datetime) -> bool:
        """
    ...
    def _check_existing_draw(
        ...
        try:
            ...
            return LotteryDraw.objects.filter(**filters).exists()
        except Exception:
            return False

    async def _scrape_game(self, game: LotteryGame) -> bool:
        """
    ...
    async def _get_scraper(self, state: str):
        ...
        if state not in self._scrapers:
            scraper_cls = get_scraper_for_state(state)
            scraper = scraper_cls()
            await scraper.start()
    ...
    @sync_to_async
    def _save_draw(self, game: LotteryGame, result: ScrapeResult) -> LotteryDraw:
    ...
    def _log_scrape(
        ...
        retry_count: int = 0,
        error_message: str = '',
        draw: Optional[LotteryDraw] = None,
        duration_ms: int = 0
    ) -> LotteryScrapeLog:
        """Log a scrape attempt."""
        return LotteryScrapeLog.objects.create(
            game=game,
            success=success,
...
async def run_daemon(interval: float = 60):
    """Convenience function to run the daemon."""
    daemon = LotteryScrapeDaemon(interval=interval)
    await daemon.start()
