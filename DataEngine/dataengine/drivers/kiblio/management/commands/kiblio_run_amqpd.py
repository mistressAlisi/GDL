...
from dataengine.drivers.kiblio.driver.daemons import KIBLAMQPd
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.MIGRATE_HEADING("KIBL.io: Starting AMPQd Daemon for {vhost}".format(vhost=vHost)))
        # print(options)
        if options["l"]:
            if options["l"][0] == "INFO":
        ...
        amqpd = KIBLAMQPd()
        amqpd.setup(vHost, logging, True)
        amqpd.run()
        amqpd.join()