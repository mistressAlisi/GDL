...
def open_tickets_table_handle(request):
    ...
    # user_ping(request)
    return render(request, "game/gdlagent/open_tickets_table.html", context)
...
def closed_tickets_table_handle(request):
    vhost, vdomain, apperance, agent, context = default_agent_dashboard_context(request)
    # user_ping(request)
    return render(request, "game/gdlagent/closed_tickets_table.html", context)
...
def open_tickets_matches_table_handle(request):
    vhost, vdomain, apperance, agent, context = default_agent_dashboard_context(request)
    # user_ping(request)
    return render(request, "game/gdlagent/open_matches_table.html", context)
