...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # -- Get Wagers ---
        self.stdout.write(self.style.MIGRATE_HEADING(f"Grader: Ungrader starting for {vHost}: Ungrading for matches with Commence Time from {start} to {stop}..."))
        loop = asyncio.new_event_loop()
        loop.run_until_complete(engine.ungrade_matches(start,stop))
